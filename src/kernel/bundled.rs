...
/// arrangement: every other pane depends on that one file. It comes back as the
/// preset delivery last wrote there, so restoring an edited `split-shell` layout
/// does not quietly switch it to `classic`.
...
/// The first `layout.lua.bak[.N]` that does not exist yet — for a restore here
/// and a switch in [`super::presets::apply`] alike.
///
/// Not a `.lua` name on purpose: nothing loads it, so a backup can never become
/// a second arrangement or a stray plugin.
pub(crate) fn free_backup_path(dir: &Path) -> PathBuf {
