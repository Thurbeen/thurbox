...
impl Database {
    ...
    // ── Per-project roles (legacy `project_roles` table) ───────────
    ...
    pub fn list_roles(&self, project_id: ProjectId) -> rusqlite::Result<Vec<RoleConfig>> {
        ...
        let roles = stmt
            .query_map(params![id_str], row_to_role_config)?
            .collect::<Result<Vec<_>, _>>()?;
...
mod tests {
    ...
    fn delete_global_role() {
        ...
        assert!(!db.delete_global_role("dev").unwrap()); // already gone
    ...
    #[test]
    fn migration_seeds_global_roles_from_project_roles() {
        let (db, pid) = setup_db_with_project("test");

        // Add a per-project role
        db.replace_roles(
            pid,
            &[RoleConfig {
                name: "reviewer".to_string(),
        ...
        // The migration already ran during open_in_memory(), but the role
        // was added after. Simulate a re-migration by inserting manually.
        ...
        let global = db.list_global_roles().unwrap();
        assert!(global.iter().any(|r| r.name == "reviewer"));
    }
}
