...
impl Database {
    ...
    /// List all global MCP servers (not per-project).
    ...
    pub fn list_mcp_servers(
        ...
        let servers = stmt
            .query_map(params![id_str], row_to_mcp_server_config)?
            .collect::<Result<Vec<_>, _>>()?;
...
mod tests {
    ...
    #[test]
    fn replace_global_mcp_servers_empty_clears() {
        let db = Database::open_in_memory().unwrap();
        db.replace_global_mcp_servers(&[McpServerConfig {
            name: "temp".to_string(),
            command: "cmd".to_string(),
        ...
        db.replace_global_mcp_servers(&[]).unwrap();
    ...
    #[test]
    fn migration_seeds_global_mcp_servers_from_project_mcp_servers() {
        let (db, pid) = setup_db_with_project("test");
        db.replace_mcp_servers(
            pid,
            &[McpServerConfig {
                name: "migrated".to_string(),
                command: "migrated-cmd".to_string(),
        ...
        // Verify the global mcp_servers table exists and we can write/read from it
        db.upsert_global_mcp_server(&McpServerConfig {
            name: "migrated".to_string(),
            command: "migrated-cmd".to_string(),
            args: vec!["--arg".to_string()],
        ...
        assert_eq!(global[0].name, "migrated");
        assert_eq!(global[0].env.get("KEY"), Some(&"VAL".to_string()));
    }
