# plugin-programs Specification

## Purpose
Defines what it takes for a plugin to hand the user a program to *use* rather
than a program's output to read: a pane the plugin owns rather than one belonging
to a session, which takes the keystrokes the focused pane does not claim, is told
the size it is drawn at, and states its own lifetime — together with the grant and
the bounds that stop an interactive process being something an existing trust
decision quietly widened into.
## Requirements
