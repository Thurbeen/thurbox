...
| 18 | `DeletedRow` is `{id, name, partial}` — restore rows drop agent, deletion age and `[wt]`. | visual | `src/kernel/snapshot.rs`, `ui/plugins/80_restore.lua` |
...
| 43 | Terminal scroll chords are **undeclared** (only `ctrl+t`/`f8` in `keys`): nothing in help, nothing rebindable, no `Shift+Up/Down`, and bare `PageUp` is consumed in `on_key` so the agent never sees CSI 5~. | behavior | `ui/plugins/20_agent.lua` |
...
| 60 | `to_press` drops `KeyModifiers::SUPER`, no kitty `DISAMBIGUATE` push, `canonical_chord` can never emit `cmd+…` — so Cmd is unbindable and, on a terminal that does report it, `Cmd+J` fires the plain `j` binding. | missing | `src/kernel/host.rs`, `src/bin/thurbox2.rs`, `src/kernel/registry.rs` |
...
| 66 | `Ctrl+C`/`Ctrl+V` are matched literally ahead of the registry (unrebindable, listed under "Fixed"); `Ctrl+C` with no selection copies nothing (v1 copies the status message); toasts carry no level, so `INFO`/`✓ SYNC`/`ERROR` badges are gone. | polish | `src/kernel/modals/mod.rs`, `src/bin/thurbox2.rs` |
...
15. **Confirm + restore-row detail.** Build `store.confirm.lines` from `session.git` (with a "could not verify" line for remote/nil) and render them; add `agent`/`deleted_at`/`worktrees` to `DeletedRow`. Fixes #17, #18. (`ui/plugins/10_sessions.lua`, `ui/plugins/60_confirm.lua`, `src/kernel/snapshot.rs`, `ui/plugins/80_restore.lua`)
