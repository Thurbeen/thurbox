#!/usr/bin/env sh
# Regenerate the main Thurbox demo media (docs/media/thurbox-demo.{gif,mp4})
# using REAL coding-agent CLIs.
#
# Unlike scripts/demo/record.sh (which uses a deterministic canned "demo agent"),
# this drives the actual `claude`, `opencode`, `codex` and `gemini` CLIs — one per
# thurbox session — to showcase real multi-agent orchestration. No prompt is sent
# to any agent; they are launched and left on their start screens.
#
...
#     have running.
#   * XDG_{DATA,CONFIG}_HOME point at a throwaway dir.
#
...
export THURBOX_BIN   # consumed by scripts/demo/agents.tape

# --- Isolated environment ----------------------------------------------------
DEMO_HOME=$(mktemp -d "${TMPDIR:-/tmp}/thurbox-agents.XXXXXX")
